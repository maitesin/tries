...
template <class T>
const T & TST::tst<T>::find(const std::string & key) {
...
template <class T>
typename TST::tst<T>::node_ptr TST::tst<T>::find(TST::tst<T>::node_ptr & n,
	...
	if (key.size() == d + 1) {
		if (n == nullptr)
			...
			if (n->c == key[d]) return find(n->middle, key, d+1);
			if (n->c <  key[d] && n->right != nullptr) {
				return find(n->right, key, d);
			}
		}
...
template <class T>
void TST::tst<T>::insert(const std::string & key,
		      const T & value) {
	if (key == "") return;
	if (root == nullptr)
...
template <class T>
typename TST::tst<T>::node_ptr TST::tst<T>::insert(TST::tst<T>::node_ptr n,
						   const std::string & key,
						   const T & value,
						   size_t d,
						   bool & created) {
...
template <class T>
void TST::tst<T>::clear(TST::tst<T>::node_ptr n) {
	if (n->left   != nullptr) clear(std::move(n->left));
	if (n->middle != nullptr) clear(std::move(n->middle));
	if (n->right  != nullptr) clear(std::move(n->right));
	n.reset();
}
...
template <class T>
bool TST::tst<T>::contains(TST::tst<T>::node_ptr & n,
			const std::string & key,
			size_t d) {
	if (key.size() == d + 1) {
...
template <class T>
void TST::tst<T>::erase(const std::string & key) {
	bool decrease = false;
...
template <class T>
bool TST::tst<T>::erase(TST::tst<T>::node_ptr & n,
			 const std::string & key,
			 size_t d,
			 bool & decrease) {
...
template <class T>
void TST::tst<T>::keys(TST::tst<T>::node_ptr & n,
			   std::string prefix,
			   size_t d,
			   TST::tst<T>::vec_ptr & v) {
	if (prefix.size() <= d + 1) {
...
template <class T>
void TST::tst<T>::gather_keys(TST::tst<T>::node_ptr & n,
			   std::string prefix,
			   TST::tst<T>::vec_ptr & v) {
	if (n == nullptr) return;
	if (n->value != def) {
...
template <class T>
void TST::tst<T>::show_label(TST::tst<T>::node_ptr &n,
			     size_t & label) {
...
template <class T>
void TST::tst<T>::show(TST::tst<T>::node_ptr &n,
			 size_t & label) {
