...
template <class T, size_t R>
typename Trie::trie<T, R>::node_ptr
Trie::trie<T, R>::find(Trie::trie<T, R>::node_ptr &n, const std::string &key,
                       size_t d) {
  if (key.size() == d) {
    if (n == nullptr) {
      return nullptr;
...
template <class T, size_t R>
void Trie::trie<T, R>::insert(const std::string &key, const T &value) {
  if (key != "") {
    bool created = false;
...
template <class T, size_t R>
typename Trie::trie<T, R>::node_ptr
Trie::trie<T, R>::insert(Trie::trie<T, R>::node_ptr n, const std::string &key,
...
bool Trie::trie<T, R>::erase(Trie::trie<T, R>::node_ptr &n,
                             const std::string &key, size_t d, bool &decrease) {
...
template <class T, size_t R>
void Trie::trie<T, R>::keys(Trie::trie<T, R>::node_ptr &n, std::string prefix,
                            size_t d, vec_ptr &v) {
  if (prefix.size() == d) {
...
template <class T, size_t R> void Trie::trie<T, R>::show() {
...
template <class T, size_t R>
void Trie::trie<T, R>::show_label(Trie::trie<T, R>::node_ptr &n, size_t pos,
                                  size_t &label) {
  std::cout << label << " [shape=record,label=\"{ <data>"
...
template <class T, size_t R>
void Trie::trie<T, R>::show(Trie::trie<T, R>::node_ptr &n, size_t pos,
                            size_t &label) {
...
template <class T, size_t R> std::string Trie::trie<T, R>::lcp() {
  return lcp(root, "");
}
