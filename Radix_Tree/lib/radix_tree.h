 ...
 * Radix Tree
 */
template <class T, size_t R> class radix_tree {
public:
  // Constructor
...
private:
   ...
   */
  node_ptr split(node_ptr n, const T &value, size_t p);

  /*
   * Auxiliar method that find where begins the
   * difference and splits there
   */
  node_ptr find_diff_and_split(node_ptr n, const std::string &key,
                               const T &value);
  ...
  void merge(node_ptr &father, node_ptr &son);
  ...
  std::string prefix_cut(std::string &prefix, size_t d);
